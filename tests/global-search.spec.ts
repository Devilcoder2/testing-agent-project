...
test("searches the authorized workspace with debounce, current-section priority, and keyboard navigation", async ({ page }) => {
  ...
  const dataSet = await prisma.testDataSet.create({ data: { productId: product.id, ownerId: owner.id, name: dataName, fieldNames: ["customer_email"], encryptedFields: "safe-browser-search-fixture", reusePolicy: TestDataReusePolicy.REUSABLE } });
