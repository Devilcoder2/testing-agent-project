...
The marketing application initially renders only the sanitized walkthrough poster. A visitor action creates the managed Stream player inside an accessible dialog; closing it destroys the player and restores focus. Real product screenshots use development fixture data only. Motion is presentation-only, lazy-loaded, bounded to small opacity/transform changes, and disabled through the reduced-motion preference. A privacy route, sitemap, robots response, canonical metadata, structured data, and one branded social card share the same public content source.
