...
Phases 1–11 remain as documented above. Phase 12 now provides a Docker-local organization boundary: database-backed eight-hour sessions, password accounts, invitation and password-reset links through Mailpit, Admin/Manager/Tester roles, Product assignments, and an Admin-only **Administration** page. The live acceptance checks cover bootstrap sign-in, invitation acceptance, Manager denial of Administration, Manager Product creation, and immediate session revocation on disable. Broader browser permission-matrix hardening and the owner learning review remain follow-up work.
