...
export function TestDataView() {
  ...
  return <div className="dashboard-grid">
    ...
    {confirmingInvalidation && <Dialog eyebrow="Test Data lifecycle" title={`Invalidate ${confirmingInvalidation.name}?`} detail="This will prevent every currently safe row from being selected for new Runs. Stored values remain protected." onClose={() => { if (!invalidatingId) setConfirmingInvalidation(null); }} actions={<><Button variant="ghost" onClick={() => setConfirmingInvalidation(null)} disabled={Boolean(invalidatingId)}>Cancel</Button><Button variant="danger" onClick={() => void invalidate(confirmingInvalidation)} disabled={Boolean(invalidatingId)}>{invalidatingId ? "Invalidating…" : "Invalidate Test Data"}</Button></>} />}
  </div>;
}
...
function TestDataEditor({ mode, products, initialProductId, dataSet, onClose, onSaved }: { mode: "create" | "edit"; products: Product[]; initialProductId: string; dataSet?: TestDataDetail; onClose: () => void; onSaved: (dataSet: TestDataSet, action: "created" | "saved") => void }) {
  ...
  return <Dialog eyebrow="Local Test Data" title={mode === "create" ? "Create Test Data" : `Edit ${dataSet?.name}`} detail="Each complete row supplies one Run. Stored cells remain masked unless you replace them." onClose={onClose} className="test-data-modal">
    <form className="form-stack" onSubmit={submit}>
      ...
      <div className="test-data-editor__toolbar"><IconButton type="button" label="Upload Excel workbook" onClick={() => fileRef.current?.click()} disabled={working}><Icon name="upload" /></IconButton><input ref={fileRef} className="visually-hidden" type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={(event) => { const file = event.target.files?.[0]; if (file) void importExcel(file); }} /><div className="test-data-editor__table-actions" role="group" aria-label="Table structure"><Button type="button" variant="secondary" onClick={addColumn} disabled={working}><span aria-hidden="true">+</span> Add column</Button><Button type="button" variant="secondary" onClick={addRow} disabled={working}><span aria-hidden="true">+</span> Add row</Button></div><StatusBadge tone="info">{rows.length} row{rows.length === 1 ? "" : "s"} · {columns.length} column{columns.length === 1 ? "" : "s"}</StatusBadge></div>
      <div className="test-data-grid-wrap"><table className="test-data-grid"><thead><tr><th scope="col">#</th>{columns.map((column, columnIndex) => <th scope="col" className={`test-data-grid__column--${column.width}`} key={column.key}><div className="test-data-grid__column-heading"><div className="test-data-grid__column-name"><TextInput aria-label={`Column ${columnIndex + 1} name`} value={column.name} onChange={(event) => renameColumn(column, event.target.value)} maxLength={64} /><IconButton type="button" className="icon-button--danger" label={`Remove column ${column.name || columnIndex + 1}`} onClick={() => removeColumn(column)} disabled={columns.length === 1 || working}><Icon name="delete" /></IconButton></div><label className="test-data-grid__width-control"><span>Width</span><select className="input select" aria-label={`Width for column ${column.name || columnIndex + 1}`} value={column.width} onChange={(event) => resizeColumn(column, event.target.value as ColumnWidth)} disabled={working}><option value="compact">Compact</option><option value="standard">Standard</option><option value="wide">Wide</option></select></label></div></th>)}<th scope="col"><span className="visually-hidden">Row actions</span></th></tr></thead><tbody>{rows.map((row, rowIndex) => <tr key={row.key}><th scope="row">{rowIndex + 1}{row.status && <small>{row.status.toLowerCase()}</small>}</th>{columns.map((column, columnIndex) => { const cell = row.cells[column.key] ?? { value: "", retained: false }; return <td className={`test-data-grid__column--${column.width}`} key={column.key}><TextInput aria-label={`Row ${rowIndex + 1}, ${column.name || `column ${columnIndex + 1}`}`} value={cell.value} placeholder={cell.retained ? "Stored value (masked)" : "Enter value"} onChange={(event) => updateCell(row.key, column.key, event.target.value)} maxLength={TEST_DATA_LIMITS.cellLength} /></td>; })}<td><IconButton type="button" className="icon-button--danger" label={`Remove row ${rowIndex + 1}`} onClick={() => removeRow(row)} disabled={rows.length === 1 || working}><Icon name="delete" /></IconButton></td></tr>)}</tbody></table></div>
