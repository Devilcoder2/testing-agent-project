...
export function AdministrationView() {
  ...
  return <section className="page-stack"><PageHeader eyebrow="Organization administration" title="People and access" detail="Invite team members, assign their role, and set the Products they may access." actions={<Button onClick={() => setInviteOpen(true)}>Invite member</Button>} />
    ...
    {loading ? <Card>Loading members…</Card> : members.length === 0 ? <EmptyState title="No members yet" detail="Invite the first team member to establish organization access." /> : <div className="card-grid member-grid">{members.map((member) => <Card key={member.id} className="member-card"><div className="section-heading"><div><h3>{member.displayName}</h3><p>{member.email}</p></div><StatusBadge tone={member.accountStatus === "ACTIVE" ? "success" : "danger"}>{member.accountStatus.toLowerCase()}</StatusBadge></div><div className="member-card__meta"><span>{member.role.toLowerCase()}</span><span>{member.products.length} Product{member.products.length === 1 ? "" : "s"}</span></div><p className="member-card__products">{member.products.length ? member.products.map((product) => product.name).join(" · ") : "No Product access"}</p><div className="member-card__actions"><Button variant="secondary" onClick={() => setEditing(member)}>Edit access</Button><Button variant={member.accountStatus === "ACTIVE" ? "danger" : "secondary"} onClick={() => setConfirming(member)}>{member.accountStatus === "ACTIVE" ? "Disable" : "Reactivate"}</Button></div></Card>)}</div>}
