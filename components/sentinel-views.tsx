...
type ProductDeletionImpact = { recordings: number; testCases: number; testCaseVersions: number; runs: number; evidenceItems: number; testDataSets: number; reviewItems: number; notifications: number; integrations: number; releasesAffected: number; activeWork: number };
...
type TestDataSet = { id: string; name: string; fieldNames: string[]; status: "SAFE" | "RESERVED" | "CONSUMED" | "INVALID"; reusePolicy: "REUSABLE" | "SINGLE_USE"; rowCount: number; safeRows: Array<{ id: string; order: number }>; createdAt?: string };
...
export function TestCaseDetailView({ testCaseId }: { testCaseId: string }) {
  ...
  async function startRun(mode: "GUIDED" | "AUTO", bindings?: Record<string, unknown>, useAllRows = false) {
    setStartingRun(mode);
    setMessage("");
    try {
      const result = await request(`test-cases/${testCaseId}/${mode === "GUIDED" ? "runs" : "auto-runs"}`, "POST", bindings ? { bindings, ...(mode === "AUTO" && useAllRows ? { useAllRows: true } : {}) } : undefined) as { run: { id: string }; runs?: Array<{ id: string }> };
      if (mode === "AUTO" && (result.runs?.length ?? 0) > 1) router.push(`/runs?queued=${result.runs!.length}`);
      else router.push(`/runs/${result.run.id}`);
    } catch (startError) {
...
function TestCaseActionMenu({ testCase, steps, generatingSuggestions, onGenerateSuggestions }: { testCase: SavedTestCase; steps: Step[]; generatingSuggestions: boolean; onGenerateSuggestions: () => void }) {
  ...
  return <div ref={menuRef} className="test-case-action-menu"><IconButton label="More Test Case actions" aria-expanded={open} aria-haspopup="menu" onClick={() => setOpen((current) => !current)}><Icon name="more" /></IconButton><div className="test-case-action-menu__panel" role="menu" hidden={!open}><Link role="menuitem" href={`/test-cases/${testCase.id}/edit`} onClick={() => setOpen(false)}>Edit Test</Link><Button role="menuitem" variant="ghost" onClick={() => { setOpen(false); onGenerateSuggestions(); }} disabled={generatingSuggestions || steps.length === 0}>{generatingSuggestions ? "Generating…" : "Generate suggestions"}</Button><Link role="menuitem" href={`/review?testCaseId=${testCase.id}`} onClick={() => setOpen(false)}>Open Review</Link><OwnershipTransfer label="Test Case" currentOwnerId={testCase.ownerId} membersPath={`products/${testCase.product.id}/members`} transferPath={`test-cases/${testCase.id}/owner`} onTransferred={() => window.location.reload()} onOpen={() => setOpen(false)} triggerClassName="test-case-action-menu__item" triggerRole="menuitem" triggerVariant="ghost" /></div></div>;
}
...
function RepositoryRouting({ testCaseId }: { testCaseId: string }) {
  ...
  if (!routing.available || activeConnections.length === 0) return null;
...
function VariableBindingDialog({ mode, productId, variables, onClose, onStart }: { mode: "GUIDED" | "AUTO"; productId: string; variables: VersionVariable[]; onClose: () => void; onStart: (bindings: Record<string, unknown>, useAllRows: boolean) => Promise<void> }) {
  ...
  const selectedPoolIds = [...new Set(Object.values(bindings).flatMap((binding) => binding.source === "POOL" && binding.dataSetId ? [binding.dataSetId] : []))];
  const selectedPool = selectedPoolIds.length === 1 ? dataSets.find((dataSet) => dataSet.id === selectedPoolIds[0]) : undefined;
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setMessage("");
    if (mode === "AUTO" && selectedPoolIds.length > 1) return setMessage("Choose one shared Test Data Set for all pooled variables in an Auto Run batch.");
    const resolved = Object.fromEntries(Object.entries(bindings).map(([name, binding]) => [name, binding.source === "POOL" && binding.dataSetId && mode === "GUIDED" ? { ...binding, dataSetRowId: rowSelections[binding.dataSetId] } : binding]));
    if (mode === "GUIDED" && selectedPoolIds.some((dataSetId) => !rowSelections[dataSetId])) return setMessage("Choose one safe Test Data row for the Guided Run.");
    setSubmitting(true);
    try { await onStart(resolved, mode === "AUTO" && selectedPoolIds.length === 1); } catch (error) { setMessage(errorMessage(error, "Could not start this Run.")); } finally { setSubmitting(false); }
  }
  return <div className="modal-backdrop" role="presentation"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="variable-binding-title"><div className="modal__header"><div><p className="eyebrow">{mode === "AUTO" ? "Autonomous replay" : "Guided Run"}</p><h2 id="variable-binding-title">Choose variable values</h2><p>Sentinel stores chosen values encrypted and shows only their source after the Run starts.</p></div><Button type="button" variant="ghost" onClick={onClose} disabled={submitting}>Close</Button></div><form className="form-stack" onSubmit={submit}>{variables.map((variable) => { const binding = bindings[variable.name]; const compatible = dataSets.filter((dataSet) => dataSet.safeRows.length > 0 && dataSet.fieldNames.includes(variable.name)); return <Card className="panel-card" key={variable.name}><Field label={variable.name}><SelectInput value={binding.source} onChange={(event) => setBindings((all) => ({ ...all, [variable.name]: { source: event.target.value as "STATIC" | "POOL" | "MANUAL" } }))}><option value="STATIC" disabled={!variable.hasStaticDefault}>Static default{variable.hasStaticDefault ? " (masked)" : " unavailable"}</option><option value="POOL">Test Data Set</option><option value="MANUAL">Manual for this Run</option></SelectInput></Field>{binding.source === "POOL" && <Field label={`Test Data Set for ${variable.name}`}><SelectInput value={binding.dataSetId ?? ""} onChange={(event) => setBindings((all) => ({ ...all, [variable.name]: { ...binding, dataSetId: event.target.value } }))}><option value="">Choose safe Test Data</option>{compatible.map((dataSet) => <option key={dataSet.id} value={dataSet.id}>{dataSet.name} · {dataSet.safeRows.length} row{dataSet.safeRows.length === 1 ? "" : "s"}</option>)}</SelectInput></Field>}{binding.source === "MANUAL" && <Field label={`Manual value for ${variable.name}`}><TextInput type="password" value={binding.value ?? ""} onChange={(event) => setBindings((all) => ({ ...all, [variable.name]: { ...binding, value: event.target.value } }))} /></Field>}</Card>; })}{mode === "GUIDED" && selectedPoolIds.map((dataSetId) => { const dataSet = dataSets.find((item) => item.id === dataSetId); return dataSet ? <Field key={dataSet.id} label={`Row from ${dataSet.name}`}><SelectInput value={rowSelections[dataSet.id] ?? ""} onChange={(event) => setRowSelections((all) => ({ ...all, [dataSet.id]: event.target.value }))}><option value="">Choose a safe row</option>{dataSet.safeRows.map((row) => <option key={row.id} value={row.id}>Row {row.order}</option>)}</SelectInput></Field> : null; })}{mode === "AUTO" && selectedPool && <Feedback tone="info">{selectedPool.safeRows.length} safe row{selectedPool.safeRows.length === 1 ? "" : "s"} will queue {selectedPool.safeRows.length} Auto Run{selectedPool.safeRows.length === 1 ? "" : "s"}.</Feedback>}{message && <Feedback tone="danger">{message}</Feedback>}<div className="modal__actions"><Button type="button" variant="ghost" onClick={onClose} disabled={submitting}>Cancel</Button><Button type="submit" disabled={submitting}>{submitting ? "Starting…" : mode === "AUTO" ? "Queue Auto Run" : "Start Guided Run"}</Button></div></form></section></div>;
}
...
function StepTimelineItem({ step }: { step: Step }) {
  ...
  const action = step.kind.toLowerCase().replaceAll("_", " ").replace(/^./, (character) => character.toUpperCase());
  const hasAnnotations = Boolean(step.description || step.expectedOutcome || step.variableName || step.isCheckpoint);
  return <article className={`timeline-item ${step.isCheckpoint ? "timeline-item--checkpoint" : ""}`}><div className="timeline-item__rail"><span className="timeline-item__number">{step.order}</span></div><details className="timeline-item__card"><summary className="timeline-item__summary"><span className="timeline-item__summary-copy"><strong>{action}</strong><span className="timeline-item__target">{label}</span>{step.value && step.value !== label && <small>{step.value}</small>}</span>{step.isCheckpoint && <span className="timeline-item__checkpoint-label">Checkpoint</span>}<Icon name="chevronDown" className="timeline-item__chevron" /></summary><div className="timeline-item__details">{step.description && <p className="timeline-item__annotation"><strong>Description:</strong> {step.description}</p>}{step.expectedOutcome && <p className="timeline-item__annotation"><strong>Expected outcome:</strong> {step.expectedOutcome}</p>}{step.variableName && <p className="timeline-item__annotation"><strong>Variable:</strong> {step.variableName}</p>}{step.isCheckpoint && <p className="timeline-item__annotation"><strong>Checkpoint:</strong> Review required during Auto Run</p>}{!hasAnnotations && <p className="timeline-item__annotation">No additional description or expected outcome was recorded.</p>}</div></details></article>;
}
