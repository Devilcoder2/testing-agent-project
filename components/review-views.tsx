...
function ChangeProposalQueue() {
  ...
  useEffect(() => { void load(); }, []);
  async function decide(proposal: ChangeProposal, action: "approve" | "reject") {
    ...
    try { await request(`change-proposals/${proposal.id}/${action}`, "POST", { note }); setMessage(`Proposal ${action === "approve" ? "approved and saved as a new Test Case version" : "rejected; a Jira draft was prepared when this Product has a Jira mapping"}.`); await load(); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not decide this proposal."); } finally { setWorkingId(null); }
  }
  return <Card className="panel-card"><div className="panel-card__head"><div><p className="eyebrow">Baseline changes</p><h2>Change proposals</h2><p>Failed Runs can propose only descriptions and expected outcomes after a known QA deployment. The source baseline remains immutable until its original owner approves.</p></div><StatusBadge tone="info">{proposals.length} visible</StatusBadge></div>{message && <Feedback tone={message.startsWith("Proposal") ? "success" : "danger"}>{message}</Feedback>}{loading ? <StatusBadge tone="info">Loading change proposals</StatusBadge> : proposals.length === 0 ? <EmptyState title="No change proposals" detail="Create one from a completed failed Run only when a known QA deployment intentionally changed its behavior." /> : <div className="review-list">{proposals.map((proposal) => <article className="review-item" key={proposal.id}><div className="review-item__main"><div className="review-item__head"><h2>{proposal.testCase.name}</h2><StatusBadge tone={proposal.status === "APPROVED" ? "success" : proposal.status === "REJECTED" || proposal.status === "STALE" ? "danger" : "warning"}>{proposal.status.toLowerCase()}</StatusBadge></div><p className="review-item__meta">Failed Run <Link href={`/runs/${proposal.run.id}`}>evidence</Link> · Source version {proposal.sourceVersion.version} · Submitted by {proposal.createdBy.displayName} · Owner {proposal.owner.displayName}</p><p><strong>Deployment context:</strong> {proposal.context}</p>{proposal.changes.map((change) => { const source = proposal.sourceVersion.steps.find((step) => step.id === change.sourceStepId); return <div className="review-item__proposal" key={change.sourceStepId}><p><strong>Step {change.order}</strong></p><p><strong>Description:</strong> {source?.description ?? "—"} <span aria-hidden="true">→</span> {change.proposedDescription ?? "—"}</p><p><strong>Expected:</strong> {source?.expectedOutcome ?? "—"} <span aria-hidden="true">→</span> {change.proposedExpectedOutcome ?? "—"}</p></div>; })}{proposal.decisionNote && <p><strong>Decision note:</strong> {proposal.decisionNote}</p>}{proposal.appliedVersion && <p><strong>Applied baseline:</strong> Version {proposal.appliedVersion}</p>}</div><div className="review-item__actions">{proposal.canDecide && proposal.status === "SUBMITTED" && <><Button onClick={() => void decide(proposal, "approve")} disabled={workingId === proposal.id}>{workingId === proposal.id ? "Working…" : "Approve change"}</Button><Button variant="danger" onClick={() => void decide(proposal, "reject")} disabled={workingId === proposal.id}>Reject change</Button></>}</div></article>)}</div>}</Card>;
}
