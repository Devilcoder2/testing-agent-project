    ...
    <Card className="detail-card">
      ...
      <form className="form-stack" onSubmit={save}>
        ...
        <div className="step-editor-list">{steps.map((step, index) => {
          ...
          return <details className="step-editor" key={step.id} open={expandedSteps.has(step.id)} onToggle={(event) => setExpandedSteps((current) => { const next = new Set(current); if (event.currentTarget.open) next.add(step.id); else next.delete(step.id); return next; })}>
