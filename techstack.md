...
| Marketing styling | Dedicated native CSS token system | Release Proof OKLCH values in `marketing-design.md` | Creates a recognizable public identity without changing the product's Signal Canvas tokens, while keeping all production colours inside named theme tokens. |
| Typography | Self-hosted Antonio and IBM Plex Sans assets through Fontsource packages | `@fontsource/antonio` and `@fontsource-variable/ibm-plex-sans`, pinned in the marketing lockfile | Pairs a condensed release-statement display face with a highly legible technical body face without a runtime font request or visitor tracking. |
