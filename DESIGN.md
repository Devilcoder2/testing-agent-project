...
- **Recording Workspace:** a standalone instrument view with a slim session header, collapsible timeline rail, and dominant browser stage. It does not inherit the global masthead. Collapse and restore use compact directional icon controls with accessible names and tooltips; the collapsed rail has no rotated text or count badge. A full-screen control hides the session header and rail so the browser stage owns the full viewport. A persistent minimize control restores the prior rail state; this is Sentinel layout mode, not browser-native full screen.
