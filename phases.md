...
- [ ] Keep the interactive preview inside the marketing bundle with immutable sample records, navigation-only state, an explicit sample/read-only label, no product imports, no credentials, no cookies, no persistence, no mutation controls, and no runtime data requests.
- [ ] Replace low-resolution or theme-inconsistent proof images, add the signal-path logo and matching favicon, and use only sanitized supporting media and approved truthful copy; omit fabricated proof, pricing, availability dates, general signup, bulk outreach, and third-party behavioral analytics.
- [ ] Add lazy bounded Motion transitions, native non-autoplay horizontal scrolling, full reduced-motion behavior, Stream player lifecycle, captions/fallback content, canonical metadata, crawl files, SoftwareApplication structured data, and a branded social preview.
...
- [ ] Verify preview navigation and immutability, feature-rail controls and dialog behavior, keyboard, focus, screen-reader semantics, captions, WCAG 2.2 AA contrast, 200% zoom, reduced motion, desktop/tablet/mobile layout, no horizontal overflow, lazy video loading, metadata, and error/success states through browser coverage and live review.
