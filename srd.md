...
Sentinel must provide a separately deployed global-English marketing surface for startup QA and engineering teams without moving or redesigning the authenticated product. Its first viewport communicates the human-taught, evidence-backed value proposition, provides a pilot-waitlist action, and exposes a clearly labelled read-only sample workspace. The page uses concise sections for the Teach → Replay → Decide workflow, implemented product capabilities, safety and human-control boundaries, pilot expectations, and frequently asked questions.

The marketing surface must use the approved Quiet Flight Workbench direction in `marketing-design.md`: a spacious near-white composition, restrained cobalt action accent, neutral modern typography, and one clear idea per visual beat. Its primary proof is an in-page sample workspace containing one representative Product, Test Case, Test Data set, Run, Release, and Review state. Visitors may navigate between those destinations, but the preview exposes no create, edit, delete, record, execute, approve, authentication, or data-entry action. It is labelled as sample data and read only, works without the authenticated application, and performs no network request. A native horizontal feature rail presents Recording, Guided Runs, Autonomous Runs, Evidence, Test Data, Release readiness, and Workflows; each card opens an accessible detail dialog over a blurred backdrop and the rail never autoplays. The site must not publish fabricated metrics, customers, testimonials, pricing, access dates, general-availability claims, or functionality not present in the current source and requirements. Walkthrough playback uses a poster-first managed player, captions, keyboard controls, a focus-safe dialog, and a non-video summary; it never autoplays audio or downloads the player before a visitor requests it.

The public pilot form requires only a name and syntactically valid email. Company and QA-team-size choice (`1`, `2-5`, `6-15`, or `16+`) are optional qualification fields and remain absent when the visitor does not provide them. A valid submission requires an empty honeypot and a server-verified, single-use Turnstile token. The public endpoint accepts requests only from the configured marketing origin, never uses cookies or credentials, enforces a small request body and edge rate limit, normalizes email, and stores one lead per configured owner organization and email. New and repeated submissions return the same HTTP 202 shape and reveal no address-existence state. Sentinel sends no confirmation email.
