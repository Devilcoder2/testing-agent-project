...
Use breadcrumbs, product and owner metadata, a version badge, and a vertical ordered Step Timeline. Saved annotations appear as readable metadata chips or labelled content, while redacted values remain visibly redacted. Editing a saved version is not implied by its appearance. Phase 4 adds a Variables panel: show canonical names, affected step numbers, and masked static-default status; never display a stored raw value. Both **Run test** and **Auto Run** open the same pre-run binding dialog when variables exist. Each row selects static, a compatible Test Data Set, or a manual entry, and inline errors explain unavailable/consumed data or rejected secret-like input. The Test Data screen labels reusable and single-use sets clearly; reusable means sequential reuse only, because an active Run reserves the set until it ends. Its desktop table editor occupies 80% of the viewport in both dimensions, uses explicit **Add column** and **Add row** buttons, and gives each column one compact/standard/wide selector rather than a cluster of ambiguous icons. Phase 7 adds a secondary **Generate suggestions** action and a Review link: generation reports created, already-known, and skipped items without leaving the source Test Case or implying an automatic run.
...
## 13. Tabular Test Data workspace

The Test Data header has no generic explanatory sentence. Its only page action is a labelled plus icon for creating Test Data. The Product filter defaults to **All accessible Products** unless a valid `productId` query parameter is present; the all-Product ledger adds Product context to each item. Inventory rows show the data-set name, Reusable/Single-use and aggregate lifecycle badges, row count, and one `Fields:` line. They do not repeat masking or reuse-policy prose.

Eligible items expose labelled Edit and Invalidate icon buttons with title tooltips. Create and edit use one wide dialog containing Product (create only), name, reuse policy, an Excel-upload icon/control, and a spreadsheet-style grid. Columns have editable canonical names and remove controls. Rows are numbered and have remove controls. Dedicated plus icons add a column or row. The table scrolls horizontally inside the dialog rather than compressing cells below a usable width; narrow screens retain reachable row/column controls. Empty tables begin with one column and one row.

Existing values are represented by masked retained cells, never decrypted text. Focusing or typing in a masked cell converts only that cell into an explicit replacement; leaving it untouched preserves the ciphertext. Imported/manual new cells display their pending plaintext only inside the unsaved dialog. Closing or cancelling clears that transient state. Upload accepts `.xlsx`, replaces the current draft only after successful local parsing, uses the first non-empty row as headers, and reports file/shape/cell errors inside the dialog. Server validation errors also render inside the dialog, never behind its backdrop.

The pre-Run variable dialog keeps static/manual selection per variable. Pooled variables select one compatible table. Guided mode additionally selects one numbered safe row. Auto mode shows the resulting safe-row count and states that it will queue that many Runs; it rejects multiple pooled tables rather than implying a Cartesian join. After a multi-row Auto submission, the user moves to Runs with queued-count feedback instead of opening one arbitrary Run detail.
