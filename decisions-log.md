...
- **Status:** Implemented locally on 2026-08-24: GitHub App authentication, Cloudflare Tunnel for optional local webhook ingress, explicit Test Case-to-repository routing, automatic analysis only for GitHub-triggered failed Auto Runs, manual repository/SHA choice for other failed Runs, and OpenAI Responses structured output with `store: false`. Focused unit, Docker integration, worker-routing, authorization, and browser checks passed. A disposable live GitHub App/OpenAI sandbox validation and the owner learning review remain pending; Sentinel keeps the integration unavailable until the required server-only configuration is present.
...
## D-039 — Replace the incremental revamp with a clean-sheet dual-theme frontend

- **Date:** 2026-08-24
- **Decision:** Supersede Phase 15's visual direction and rebuild the rendered frontend from a blank design foundation while treating the current application only as a functional specification. Adopt the `DESIGN.md` Signal Canvas system, a command masthead with grouped section navigation, and equal light and dark themes. Follow system theme by default and persist an explicit local choice. Keep the existing custom React/CSS approach and local fonts/icons rather than adding a UI, theme, font, or animation library.
- **Reason:** The owner clarified that “revamp” means a new interface from the start, not refinement of the existing sidebar-led dark design. Reusing the former layout, palette, cards, and hierarchy would fail that intent even if the result were more polished. Existing business logic is mature and should remain the stable foundation while visual structure is replaced.
- **Impact:** Phase 16 may replace DOM composition, navigation presentation, CSS, semantic tokens, theme bootstrapping, and presentation-focused browser selectors across every delivered route, including Phase 13 GitHub/source analysis. It must not change API contracts, persisted data, authorization, Run/Release behavior, evidence and redaction boundaries, or integration semantics. Phase 15 remains historical documentation and is not an implementation constraint.
- **Status:** Implemented and pushed on 2026-08-24. Lint, type-check, production build, theme persistence, responsive navigation, reduced-motion behavior, and all 13 browser workflows not requiring a second live Guided Run passed. The Docker unit suite passed 48/50 assertions; the remaining two correctly received HTTP 409 because the existing user-owned Guided Run still holds the single live-browser boundary. Owner visual/usability and learning reviews remain pending.

## D-040 — Isolate generated Next.js output between host and Docker processes

- **Date:** 2026-08-24
- **Decision:** Keep the repository bind mount for live source updates, but mount separate Docker volumes at `/app/.next` for the Sentinel development server and worker. Host production builds, the container development compiler, and the worker must never share one generated `.next` directory.
...
- **Status:** Implemented and pushed on 2026-08-27. A clean Docker-local compiler met every route budget, the standard 18-page production build passed, and all four frontend/global-search browser workflows passed. Owner answers to the refinement learning check remain pending.

## D-045 — Enforce semantic hidden state for Step Log regions

- **Date:** 2026-08-27
- **Decision:** Keep the existing mounted expanded and collapsed Step Log regions, but add a component-scoped CSS rule that makes either region `display: none` when its semantic `hidden` attribute is present.
...
## D-047 — Use a shared subgrid for Product health rows

- **Date:** 2026-08-28
- **Decision:** Define the four desktop Health overview tracks on the parent ledger and make every Product row span and inherit those tracks with CSS subgrid. Keep Product names and metric/status cells non-wrapping, with visual ellipsis only on overflowing Product names.
- **Reason:** Each row previously ended with an independent `auto` status track. Different badge widths such as `Passed`, `Failed`, `Interrupted`, and `No Runs` therefore recalculated the preceding fractional tracks and shifted Tests and pass-rate starts by as much as about 33px. A parent subgrid provides one sizing calculation for every row without changing markup or data.
- **Impact:** Product, Tests, pass-rate, and status starts are identical across all desktop rows; row heights remain 56px. The complete Product name remains in the accessible button text, while exceptionally long visible names may ellipsize. Selection, filtering, compact mobile rows, health calculations, APIs, and authorization are unchanged.
- **Status:** Implemented and pushed on 2026-08-28. Live measurement across ten Products produced one unique start for each of four columns and one unique row height. Lint, strict TypeScript, the 18-page production build, and the focused dashboard/notification workflow pass. Owner learning answers remain pending.

## D-048 — Centralize protected-client session expiry handling

- **Date:** 2026-08-28
- **Decision:** Route every protected browser API request through one client boundary. The first HTTP 401 starts one best-effort keepalive logout, immediately replaces the current history entry with `/`, and leaves concurrent callers pending while the document unloads. Public account requests opt out; HTTP 403 and other errors continue to their feature UI.
- **Reason:** The server correctly expires sessions after eight hours, but independent page request helpers treated the resulting 401 payload as ordinary feedback. Text-based redirect heuristics also confused authenticated permission messages containing “access” with expired authentication. Status-aware central handling makes the transition consistent across workspace, search, recording, Run, Release, Review, notifications, Test editing, and administration surfaces.
- **Impact:** Session duration, server authorization, cookies, API status codes, and account policy do not change. `window.location.replace` prevents the stale protected view from becoming the immediate Back destination, logout clears any remaining HttpOnly cookie, simultaneous 401 responses cannot issue repeated logout requests, invalid credentials remain inline, and authenticated permission denials remain in context.
- **Status:** Implemented and pushed on 2026-08-28. Lint, strict TypeScript, and the 18-page production build pass. Focused browser coverage passes both expired-session and error-separation workflows; rapid navigation passes, and global search passes in isolation after one combined-run data-wait timeout. Owner learning answers remain pending.

## D-049 — Retire Products through an Admin-only durable deletion job

- **Date:** 2026-08-28
- **Decision:** Show Edit and Admin-only Delete as compact labelled icons, move secondary Product actions into a three-dot menu, and execute permanent Product deletion through one durable BullMQ job. The server computes the warning counts, requires exact `DELETE`, cancels active Product work, removes MinIO evidence before relational data, deletes Product-owned Test Cases/Runs/reviews/notifications/integrations, removes only the Product's items from Releases, repairs affected Release Run readiness, and retains the Release, deletion request, and audit history.
- **Reason:** Text actions overwhelmed every Product row, while a synchronous multi-table cascade would freeze the page, encourage duplicate submissions, and risk database records outliving failed evidence cleanup. Product retirement also needs a stricter authorization and confirmation boundary than normal editing.
- **Impact:** Only organization Admins receive the delete control or APIs. A persistent queued/processing/completed/failed state survives navigation and worker restart; retries use a stable job identity and ordered, bounded cleanup. Deletion is permanent and has no restore/archive path. A failed evidence deletion prevents the relational transaction, and active Auto Runs that cannot stop safely cause a retry rather than an unsafe partial delete.
- **Status:** Implemented, migrated, and pushed on 2026-08-28. The production build passes; focused database/API coverage completed a real Product cascade including MinIO cleanup and Release preservation in 3.17 seconds; both Product browser workflows pass in 18.7 seconds. Owner answers to the Phase 22 learning check remain pending.

## D-050 — Use progressive disclosure on saved Test Case detail

- **Date:** 2026-08-28
- **Decision:** Keep Product, owner, and recorded-step count in the Test Case header; present Guided Run, Auto Run, and overflow as labelled icon controls; keep secondary actions in an outside-dismissable text menu; hide repository routing unless at least one active Product repository exists; and render each immutable step as a compact native disclosure with a distinct checkpoint treatment.
- **Reason:** The previous page repeated generic and metadata copy, gave every action equal textual weight, exposed an empty optional-integration card, and forced users to scan every annotation for every recorded step. Progressive disclosure keeps the common action and step identity visible while preserving detail on demand.
- **Impact:** Accessible names and native disclosure semantics remain available to keyboard and assistive-technology users. Ownership transfer is portalled outside the dismissing menu so its dialog remains open. Test Case, Run, suggestion, ownership, GitHub, immutable-version, API, authorization, and persistence behavior are unchanged; this is a presentation and interaction refinement only.
- **Status:** Implemented and pushed on 2026-08-28. Lint, strict TypeScript, the 18-page production build, the focused Test Case workflow, Release, Suggestions, recording, Auto Run, and no-integration GitHub regressions pass. The Guided Run regression reached the existing `Another local browser session is active` guard and therefore could not start a second live session. Owner learning answers remain pending.

## D-051 — Use one private-chat Telegram assistant with a restricted public gateway

- **Date:** 2026-08-28
- **Decision:** Phase 14 uses Telegram only, through one shared deployment-owned bot. A signed-in user links one private chat with a one-time ten-minute portal deep link. The optional Telegram tunnel reaches an Nginx gateway that forwards only the Telegram webhook path; it never exposes the local Sentinel web application. Chat interaction is limited to guided opaque buttons that browse authorized Tests/Release contents and start eligible individual Auto Runs after a five-minute confirmation.
- **Reason:** Telegram provides a small sandboxable provider surface while preserving Sentinel as the authoritative product. One bot avoids per-organization provider provisioning, and the dedicated route gateway prevents an integration convenience feature from accidentally publishing the entire local pilot. Guided buttons and server-side selection state avoid treating untrusted free-form chat text as an automation command or data channel.
- **Impact:** The implementation stores encrypted chat identifiers plus a deterministic lookup hash and only safe update/selection/delivery metadata for thirty days; no provider payload or message text is retained. Every action re-checks user, organization, role, Product access, current Test version, target policy, no-checkpoint rule, and static-only variable eligibility. Telegram cannot start Release batches, supply values, expose evidence or links, cancel confirmed Runs, or alter Run/evidence truth. Incoming updates are limited to 30/minute per identity; terminal deliveries are paced at 1/second/chat and retry once for transient failures.
- **Status:** Approved for implementation on 2026-08-28. A real BotFather sandbox bot, HTTPS webhook URL, and untracked server-only credentials remain required for final provider acceptance.

## D-052 — Gate Administration before rendering protected controls

- **Date:** 2026-08-29
- **Decision:** Resolve the server-managed session in the Administration route and redirect any unauthenticated or non-Admin user to the Dashboard before rendering the administration view.
- **Reason:** The existing APIs already denied non-Admins, but the client page could still render administrative structure while its data calls failed. Route-level gating prevents even transient exposure of invitation, member, or Telegram webhook controls and gives a clearer navigation result.
- **Impact:** API authorization remains the authoritative backstop and retains its `403` responses for direct requests. Admin behavior, session lifetime, organization roles, and Product access do not change. The new Phase 14 browser check confirms a Tester cannot reach the screen or Telegram administration API.
- **Status:** Implemented and pushed on 2026-08-29. Focused lint, strict TypeScript, and the two-check Account/Admin Telegram browser workflow pass.

## D-053 — Model Test Data as encrypted ordered rows and batch one Run per row

- **Date:** 2026-08-29
- **Decision:** A Test Data set is a named Product-scoped table whose ordered `TestDataRow` records independently own encrypted cell values, lifecycle state, and Run reservation. Stored values remain write-only: edit responses expose only row identity, order, status, and masked field presence; a blank masked edit retains the existing ciphertext inside the server transaction. Auto Run may consume every safe row from exactly one pooled table and creates one isolated Run per row. Guided Run selects one safe row because the deployment has one live-browser boundary. `.xlsx` files are parsed in the browser from the first worksheet and are never uploaded.
- **Reason:** Putting multiple logical records into one encrypted payload cannot support per-row Run counts, safe concurrent reservation, partial lifecycle changes, or useful spreadsheet editing. A row entity makes those guarantees explicit. Server-side masked retention preserves the established no-reveal security boundary, and limiting batch mode to one pooled table prevents an implicit Cartesian-product policy that the owner did not request.
- **Impact:** Existing sets migrate to row 1 without decrypting or re-encrypting their payload, and historical bindings gain the corresponding row ID. Existing clients that omit `useAllRows` still queue one Run. Tables are capped at 50 columns, 1,000 rows, 500 characters per cell, and 2 MiB per workbook. Only all-safe tables can be structurally edited; reserved rows block edits/invalidation. `read-excel-file` is loaded only when import is requested, while shared numeric limits live in a browser-safe module so Node encryption never enters the client bundle.
- **Status:** Implemented, migrated, verified, and pushed on 2026-08-29. All 10 pre-existing sets and all 6 historical set bindings were preserved. Focused API tests pass 2/2, Test Data/variable browser journeys pass 3/3, live Excel import populated two rows and two columns, lint/type-check/19-page production build pass, and the applicable complete service suite passes. Owner learning answers remain pending.

## D-054 — Keep dense management views scannable with local interaction state

- **Date:** 2026-08-30
- **Decision:** Product overflow actions use a controlled menu whose panel exists in the DOM only while open and closes on outside pointer/focus or Escape. Administration cards show only member identity, email, user type, and Product count while retaining account state in the Disable/Reactivate action and complete Product assignments only in the editor. Test Data occupies 80% of the desktop viewport in both dimensions, uses visible row/column labels, provides one compact/standard/wide selector per column, keeps that width state in the open editor only, and asks for explicit confirmation before issuing the existing invalidation request.
- **Reason:** Native disclosure menus did not meet the expected dismissal behavior, Product-name lists made member cards difficult to scan, identical plus icons obscured table actions, and immediate invalidation provided insufficient protection against an accidental lifecycle change.
- **Impact:** No API, authorization, encryption, Test Data lifecycle, stored-table schema, or member-editing contract changes. Column widths are deliberately temporary presentation state, so they do not create a per-user persistence or privacy surface. Cancelling invalidation makes no network request; confirmation continues through the existing authorized endpoint.
- **Status:** Corrected, live-browser verified, and pushed on 2026-08-30. The Product page reports zero menu panels while closed, one visible panel while open, and zero after an outside click. The first member card contains no Product names while Edit access retains 20 Product choices. At a 1280 × 720 viewport the Test Data dialog measures 1024 × 576 (80% × 80%); Add row/Add column, all three width choices, and confirmation/Cancel behavior are visible and functional. Lint, strict TypeScript, and the 19-route production build pass. Owner learning answers remain pending.

## D-055 — Separate the public pilot narrative from the authenticated product

- **Date:** 2026-08-30
- **Decision:** Phase 26 places a separately deployable Editorial Signal marketing application under `marketing/` while leaving Sentinel's existing root sign-in and product routes unchanged. The public site targets global startup QA and engineering teams with one honest pilot-waitlist conversion, real sanitized product proof, and no fabricated metrics, customers, pricing, access dates, or general-availability claim. It submits through a no-credentials Sentinel endpoint that verifies Turnstile server-side and stores one lead per configured owner organization and normalized email.
- **Reason:** A public acquisition surface has a different audience, performance profile, visual identity, content lifecycle, and threat model from an organization workspace. Keeping it independently deployable avoids exposing or reshaping authenticated product navigation. Organization-owned lead storage and same-shape duplicate responses keep qualified outreach usable without introducing a marketing database, general account signup, email automation, or address enumeration.
- **Impact:** The marketing app may know only public origins, a Turnstile site key, a Stream video ID, and approved public content. Sentinel stores safe lead qualification fields and exposes them only to Admins in the exact owner organization. Cloudflare Stream and Turnstile become bounded external dependencies, recorded in `risk-log.md`; managed media or challenge failure degrades to explicit fallback/error states and never bypasses validation. Motion 13.1.1 is limited to lazy, reduced-motion-aware presentation. Public rollout requires production legal/contact content and provider configuration and does not close Sentinel's existing product shipping gate.
- **Status:** Approved for implementation on 2026-08-30. Phase 26 requirements, architecture, phases, stack, design, risk, implementation, verification, and learning evidence remain governed by the one-file commit-and-push rule.

## D-056 — Replace Editorial Signal with the Release Proof workbench

- **Date:** 2026-08-31
- **Decision:** Replace the rejected warm-ivory/vermilion Editorial Signal presentation with a custom Release Proof direction. The public page uses an ultramarine proof field, a chartreuse signal accent, Antonio statement typography, IBM Plex Sans body typography, a Workbench macrostructure, edge-aligned minimal navigation, a continuous preflight record, and a statement footer. Real sanitized Sentinel screenshots remain the dominant proof. Equal feature cards, generic SaaS navigation/footer shapes, decorative section eyebrows, fake browser chrome, gradient/glass effects, and red brand fields are prohibited.
- **Reason:** The first landing implementation was accurate but visually interchangeable with common generated startup sites. Release Proof ties the composition to Sentinel’s actual release-decision workflow: evidence is treated as a dossier being assembled, the product appears in the first viewport, and the visual trace carries a meaningful state rather than decorating empty space. Hallmark and Impeccable were installed and used for anti-template constraints, comp generation, direction selection, and reproduction checks. UI.sh’s official private design skill could not be installed without the owner’s subscription token, so only its public design principles are applied.
- **Impact:** The acquisition, public-data, waitlist, Stream, Turnstile, accessibility, and deployment contracts do not change. The marketing app changes theme tokens, fonts, structure, motion, metadata artwork, and responsive presentation. The product application remains visually and operationally untouched. The chosen comp is a design target, not a source of product facts; generated interface imagery is replaced with the real sanitized Sentinel capture before implementation.
- **Status:** Approved after owner rejection of the first direction on 2026-08-31. Documentation is synchronized before redesign implementation; final acceptance still requires comp-reproduction review, responsive/browser verification, the Hallmark slop test, the Impeccable detector/reviewer, and the Phase 26 learning gate.

## D-057 — Supersede Release Proof with the Quiet Flight product-cinema system

- **Date:** 2026-08-31
- **Decision:** Fully replace the rejected Release Proof workbench with Quiet Flight, a near-white Product Cinema system built from the shared design DNA of the owner's Apple Mac, Google Antigravity, Codex, PayPal, PhonePe, and Reflect references. The page uses short confident copy, generous negative space, one restrained cobalt action color, neutral Geist display type, IBM Plex Sans body type, and large sanitized Sentinel captures as the dominant proof. It explicitly removes the ultramarine field, chartreuse trace, Antonio condensed statements, technical dossier framing, dense annotations, and continuous preflight ledger.
- **Reason:** The Release Proof direction overexpressed Sentinel's operational metaphor and made the marketing page feel louder and denser than the product promise. The selected references consistently prioritize a concise promise, immediate product demonstration, large visual pauses, and one clear action. Quiet Flight preserves Sentinel's specific mechanism while moving differentiation into the real interface, pacing, and human-control story rather than decorative styling.
- **Impact:** Product truth, routes, waitlist behavior, public-data boundaries, Stream and Turnstile contracts, accessibility targets, and launch gates remain unchanged. Marketing tokens, typography, layout, copy density, motion, metadata artwork, and responsive presentation change. D-056 remains as historical evidence but no longer governs implementation.
- **Status:** Approved by the owner through the explicit request for a full reference-led revamp. Final acceptance still requires a meaningful preview, complete responsive implementation, browser and accessibility verification, Hallmark and Impeccable reviews, and the Phase 26 learning gate.

## D-058 — Replace screenshot-led proof with a standalone interactive workbench

- **Date:** 2026-09-01
- **Decision:** Evolve Quiet Flight from Product Cinema into Quiet Flight Workbench. The marketing bundle will contain a clearly labelled, read-only Sentinel sample workspace with local representative records and navigation-only state; a horizontal feature rail with accessible detail dialogs; restrained interaction-led motion; and a new signal-path logo used in the header and favicon. Primary product proof will no longer depend on the low-resolution screenshot set.
- **Reason:** The existing captures lose detail, one Run capture contains a mismatched dark region, and static images cannot demonstrate Sentinel's information structure. A local simulation provides the useful interaction quality of the owner's Cursor reference and the feature-discovery rhythm of the Apple iPad reference without requiring the authenticated product, a privileged demo account, or a second deployment.
- **Impact:** Only the marketing application is deployable for this experience. The preview has no credentials, cookies, API calls, persistence, mutations, or production imports; sample data is visibly disclosed. Native horizontal scrolling, dialog semantics, focus restoration, backdrop blur, bounded Motion transitions, and reduced-motion behavior become acceptance requirements. The waitlist sign-off no longer contains the sentence “no launch-date or access promise,” while the broader truthful no-guarantee contract remains unchanged.
- **Status:** Approved by the owner's explicit interaction, motion, feature-gallery, copy, and identity requests. Implementation and browser evidence remain required before this revision is complete.

## D-059 — Use progressive one-time reveals and explicit viewport geometry

- **Date:** 2026-09-01
- **Decision:** Reveal below-fold marketing groups once as they enter the viewport, using small 18px directional travel and a short blur that collapses to opacity-only behavior for reduced motion. Keep the server-rendered page visible before hydration. Give the header three equal grid tracks so the Sentinel wordmark is mathematically centered while the compact mark and pilot actions remain at opposite edges. Add top breathing room inside the horizontal feature rail and explicitly center its native dialog with fixed inset geometry.
- **Reason:** The revised landing page had little perceptible scroll response, the feature-card lift clipped against the rail's top edge, the dialog depended on inconsistent browser placement, and the combined mark/word label could not satisfy the requested split header composition. The chosen changes improve polish without adding continuous scroll effects, autoplay, layout jumps, or another interaction dependency.
- **Impact:** Hero content remains immediately visible and crawlable; below-fold content progressively enhances after hydration and reveals only once. Reduced-motion visitors receive no translation or blur. Feature content, sample-data boundaries, dialog focus restoration, pilot conversion, and product deployment remain unchanged. The local Cloudflare worker inspector is disabled because the marketing preview does not use it and a broad debug listener is unnecessary.
- **Status:** Implemented, pushed, and verified on 2026-09-01. Lint and strict TypeScript pass, and ten desktop/mobile browser checks confirm scroll reveal, centered Geist wordmark, visible hover border, centered dialog, focus restoration, reduced motion, zoom, preview navigation, lazy walkthrough, and waitlist behavior.

## D-060 — Trigger reveals inside the attention zone and align media to the shared canvas

- **Date:** 2026-09-01
- **Decision:** Exclude the bottom 20% of the viewport from the scroll-reveal observer and require 8% of a content group inside the remaining observation area before revealing it. Force the walkthrough poster and its animation wrapper to fill the complete media grid track. Start the feature rail at the same centered-canvas inset as its heading while allowing the rail to continue to the viewport's right edge.
- **Reason:** Full-viewport observation allowed animations to complete below the visitor's current attention, the animation wrapper let the native button shrink instead of filling its grid column, and grid padding did not establish the first horizontal card's start position consistently. The new rules express the intended geometry directly and remain responsive without changing content or page structure.
- **Impact:** Reveals still run once and preserve visible server HTML and reduced-motion behavior. The walkthrough returns to its earlier 16:9 prominence. The first capability card now aligns with the heading at centered wide widths and normal page-gutter widths. No waitlist, media-loading, feature-dialog, or product-preview behavior changes.
- **Status:** Implemented, pushed, and verified on 2026-09-01. Ten desktop/mobile browser checks pass, including explicit pre-gate concealment, post-gate reveal, poster-to-grid width equality, and heading-to-first-card alignment within two pixels.

## D-061 — Put feature gutters inside a full-width free-scrolling track

- **Date:** 2026-09-01
- **Decision:** Keep the feature rail itself at full viewport width and place the cards in a max-content flex track with equal leading and trailing padding derived from the shared page inset. Remove mandatory scroll snapping while retaining native momentum, keyboard scrolling, and smooth previous/next controls.
- **Reason:** Applying the left inset to the scrolling viewport clipped cards at that inset throughout the journey and provided no matching terminal gutter. Putting both gutters on the inner track lets the first card begin on the content line, lets intermediate cards travel to the viewport edge, and lets the final card stop with an equal right gutter. Mandatory snap consumed the leading padding on load or advanced to the next snap candidate, so it conflicted with the required initial geometry.
- **Impact:** Card content, dialogs, controls, responsive card sizes, accessibility, and feature ordering are unchanged. Pointer and touch scrolling are now continuous instead of snapping. Desktop and mobile browser checks explicitly measure initial alignment, an intermediate card at the viewport edge, and the final trailing gutter within two pixels.
- **Status:** Implemented, pushed, and verified on 2026-09-01. Lint, strict TypeScript, production build, the focused two-browser rail check, and all ten landing-page browser journeys pass. Owner learning answers remain pending.

## D-062 — Minimize pilot qualification and keep the landing header in document flow

- **Date:** 2026-09-01
- **Decision:** Require only name and email for a pilot application. Store company and QA-team size as nullable qualification fields, display explicit “not provided” states to administrators, and let a later duplicate submission refresh those optional values to null when omitted. Keep the marketing header in normal document flow instead of pinning it to the viewport.
- **Reason:** Requiring organizational details adds friction before a visitor has established trust, while inventing fallback values would corrupt qualification data. A non-sticky header preserves more reading space and matches the owner's final review of the landing-page rhythm.
- **Impact:** Turnstile, honeypot, rate limiting, CORS, duplicate-address protection, owner-organization isolation, lifecycle status, and manual follow-up remain unchanged. Existing populated leads retain their values until resubmitted; new minimal leads store null qualification. The header remains fully available at page entry and scrolls away with the opening section.
- **Status:** Implemented, migrated, pushed, and verified on 2026-09-01. The focused API suite passes 10/10; marketing lint, strict TypeScript, production build, and all ten desktop/mobile browser journeys pass. Owner learning answers remain pending.

## D-063 — Preserve recorder causality and recover only orphaned Guided Runs

- **Date:** 2026-09-02
- **Decision:** Send recorder events through one serial browser-side delivery queue, rather than allowing independent keepalive requests to race into persistence. A Guided Run database record is a lock only when the current Sentinel process owns the matching local browser; a `RUNNING` Guided Run without that owner is finalized as interrupted, releases its reserved Test Data, and receives an audit event before a new Guided Run starts.
- **Reason:** An immediate redirect could reach the recorder API before the click request that caused it, reversing recorded steps and breaking replay. The reported local stack also contained an August 22 `RUNNING` Guided Run after the Sentinel and Selenium services restarted with no Selenium session, creating a permanent false browser-busy warning.
- **Impact:** Existing capture kinds, password redaction, selector semantics, browser-slot concurrency, and the active-session safety boundary remain unchanged. A genuine in-memory Guided Run still blocks a second Guided Run; only state that cannot control the current browser is recovered. Interrupted recovery returns a previously reserved Test Data row to `SAFE`, matching ordinary interruption behavior.
- **Status:** Implemented and pushed on 2026-09-02. Focused Guided Run integration coverage verifies both orphaned-run recovery and the genuine live-session block; the full remote recording journey verifies Sign in persists before its redirect; and the Selenium-lock regression verifies target policy plus untracked-session reclaim. Owner learning answers remain pending.

## D-064 — Keep Product feedback transient and GitHub configuration contained

- **Date:** 2026-09-02
- **Decision:** Render the Product GitHub settings through the shared focus-trapping dialog in a portal above the workspace shell. Permit dismissal only from Escape, the explicit close controls, or a pointer event whose target is the backdrop itself. Give that dialog a fixed header and footer with a separately scrolling content region. Keep durable deletion requests in the API/history, but surface a completed Product-deletion acknowledgement only when this Products page observes its own queued or processing request become completed; clear that acknowledgement after five seconds and omit historical completed requests when the page first loads.
- **Reason:** The menu-owned GitHub dialog was unmounted whenever its parent menu processed a pointer event outside itself, so interacting with the modal could close it. Its non-portalled stacking context could also sit behind the sticky shell, and whole-dialog scrolling made close controls difficult to reach. Completed deletion requests are durable by design, but using their full history as a page banner made old confirmation text reappear forever after refreshes or later navigation.
- **Impact:** GitHub connection, editing, pause/resume, disconnect, branch rules, analysis controls, server configuration, authorization, and API contracts are unchanged. Dialog focus restoration and Escape support remain shared behavior. Deletion audit retention, worker polling, failure visibility, Release preservation, and background-job semantics are unchanged; only the completed visual acknowledgement now expires. No browser-native fullscreen, new dependency, storage persistence, or business-rule change is introduced.
- **Status:** Implemented and pushed on 2026-09-02. The focused GitHub dialog regression passes for modal containment, internal scroll, stacking order, inside click persistence, and backdrop dismissal. The focused Product deletion regression passes for queued/completed flow, five-second removal, and no completion message after reload. Repository-wide lint/type-check are separately blocked by pre-existing generated marketing lint inputs and missing marketing components.
