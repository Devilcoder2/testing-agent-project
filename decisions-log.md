...
- **Date:** 2026-08-30
- **Decision:** Product overflow actions use a controlled menu that closes on outside pointer/focus and Escape. Administration cards show member identity, email, account state, user type, and Product count while retaining complete Product assignments only in the editor. Test Data opens in a wide dialog, uses visible row/column labels for structural actions, keeps column compact/expanded state in the open editor only, and asks for an explicit confirmation before issuing the existing invalidation request.
...
- **Status:** Implemented and pushed on 2026-08-30. Lint, strict TypeScript, and the 19-route production build pass. Focused browser runs were started but exceeded the existing 30-second test timeout while loading the large local fixture workspace; their artifacts contain no assertion failure in the new controls. Owner learning answers remain pending.
